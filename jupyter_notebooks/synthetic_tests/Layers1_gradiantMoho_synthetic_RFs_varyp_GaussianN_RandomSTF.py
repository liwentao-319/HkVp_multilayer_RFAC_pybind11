...
SynSavePath = f"synthetic_data_S{freqmax}/Varyp_crust_mantle_gradMoho_synthetics_c{Thickness_crust}_Vpc{Vpc}_kc{Kc}_kcl{Kcl}_thicknesscl{thickness_cl}_dt{dt}"
if not os.path.isdir(SynSavePath):
    os.makedirs(SynSavePath)
if thickness_cl!=0:
    dh_gradient = thickness_cl/Nsedilayer
    thicks_gradient = np.ones([Nsedilayer])*dh_gradient
    depth_gradient = np.cumsum(thicks_gradient)
    Vpcl_gradient = Vpc+(Vpm-Vpc)/thickness_cl*depth_gradient
    Vss =  np.concatenate([np.array([Vpc])/Kc,Vpcl_gradient/Kcl,np.array([Vsm])]) 
    Vps =  np.concatenate([np.array([Vpc]),Vpcl_gradient,np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([Thickness_crust]),thicks_gradient,np.array([0])]) 
else:
    Vss =  np.concatenate([np.array([Vpc])/Kc,np.array([Vsm])]) 
    Vps =  np.concatenate([np.array([Vpc]),np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([Thickness_crust]),np.array([0])]) 
