...
def random_source_time_fun(size, freqmin, freqmax, delta):
...
thickness_c = 35
k = 1.9
SynSavePath = f"Varyp_sediment_crust_mantle_synthetics_c{thickness_c}_k{k}_dt{dt}_f{freqmin}_f{freqmax}"
...
for snr in snrs:
    ...
    SynSavePath1 = f"{SynSavePath}/sediments_depth{depth_s}_snr{snr}_ST{STmean}"
    ...
    for ray_param in ray_params:
        ...
        source_fun = random_source_time_fun(N_STmean, freqmin, freqmax, dt)
