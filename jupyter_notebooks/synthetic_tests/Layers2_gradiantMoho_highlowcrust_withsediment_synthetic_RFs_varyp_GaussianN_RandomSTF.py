...
for param in params:
    ...
    for thickness_s in thickness_ss:
        ...
        vps = 3.6
        # thickness_s = 4
        SynSavePath = f"synthetic_data_S{freqmax}/Varyp_crust_mantle_gradMoho_synthetics_c{Thickness_crust}_Vpc{Vpc}_kc{Kc}_Vplc{Vplc}_klc{Klc}_tlc{thickness_lc}_hs{thickness_s}_Vps{vps}_Ks{ks}_dt{dt}"
        if not os.path.isdir(SynSavePath):
            os.makedirs(SynSavePath)

        Vss =  np.concatenate([np.array([vps/ks,Vpc/Kc,Vplc/Klc]),np.array([Vpm])/Km]) 
        Vps =  np.concatenate([np.array([vps,Vpc,Vplc]),np.array([Vpm])]) 
        Rhos =   Vp2rho(Vps)
        Thicks = np.concatenate([np.array([thickness_s,Thickness_crust-thickness_lc,thickness_lc]),np.array([0])]) 
        print(Vps)
        print(Vss)
        print(Thicks)
        Vp = Vps.astype('float')
