...
SynSavePath = f"synthetic_data_S1/Varyp_gradientsediment_crust_mantle_synthetics_c{thickness_c}_ks{ks}_dt{dt}"
...
depth_s = 2
vp1 = 2.7 
vp2 = 4.6 ###average of Vp is 3.6
Nsedilayer = 40
dh_gradient = depth_s/Nsedilayer
thicks_gradient = np.ones([Nsedilayer])*dh_gradient
depth_gradient = np.cumsum(thicks_gradient)-dh_gradient
Vps_gradient = vp1+(vp2-vp1)/depth_s*depth_gradient
Rhos_gradient = np.ones([Nsedilayer])*2.27
Vss =   np.concatenate([Vps_gradient/ks,np.array([    6.7/1.753, 4.5])]) 
Vps =   np.concatenate([Vps_gradient   ,np.array([    6.7,       8.0])]) 
Rhos =  np.concatenate([Rhos_gradient  ,np.array([    2.7,       3.3])])
Thicks =np.concatenate([thicks_gradient,np.array([thickness_c,    0])])
