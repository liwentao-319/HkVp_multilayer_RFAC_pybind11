...
SynSavePath = f"synthetic_data_S{freqmax}/Varyp_crust_mantle_gradMoho_synthetics_c{Thickness_crust}_Vpc{Vpc}_kc{Kc}_kcl{Kcl}_thicknesscl{thickness_cl}_thicknesssedi{thickness_s}_Vps{Vps}_Ks{Ks}_dt{dt}"
...
if thickness_cl!=0:
    ...
    Vss =  np.concatenate([np.array([Vps,Vpc])/Kc,Vpcl_gradient/Kcl,np.array([Vpm])/Km]) 
    Vps =  np.concatenate([np.array([Vps,Vpc]),Vpcl_gradient,np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([thickness_s,Thickness_crust]),thicks_gradient,np.array([0])]) 
else:
    Vss =  np.concatenate([np.array([Vps,Vpc])/Kc,np.array([Vpm])/Km]) 
    Vps =  np.concatenate([np.array([Vps,Vpc]),np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([thickness_s,Thickness_crust]),np.array([0])]) 
