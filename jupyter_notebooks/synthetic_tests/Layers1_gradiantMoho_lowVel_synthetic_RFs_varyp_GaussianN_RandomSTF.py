...
SynSavePath = f"synthetic_data_S{freqmax}/Varyp_crust_mantle_gradMoho_synthetics_c{Thickness_crust}_Vpc{Vpc}_kc{Kc}_kcl{Kcl}_Tcl{thickness_cl}_TlV{thickness_lowVel}_THV{thickness_highVel}_dt{dt}"
if not os.path.isdir(SynSavePath):
    os.makedirs(SynSavePath)
if thickness_cl!=0:
    Vpcl,Vscl,thicks_cl = designMOhotransition(thickness_cl,thickness_lowVel,thickness_highVel,Vpc,Vpm,Vpc/Kcl,Vpm/Km,Vpc,Vpc/Kc,20)
    Vss =  np.concatenate([np.array([Vpc])/Kc,Vscl,np.array([Vpm])/Km]) 
    Vps =  np.concatenate([np.array([Vpc]),Vpcl,np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([Thickness_crust-thickness_cl]),thicks_cl,np.array([0])]) 
else:
    Vss =  np.concatenate([np.array([Vpc])/Kc,np.array([Vpm])/Km]) 
    Vps =  np.concatenate([np.array([Vpc]),np.array([Vpm])]) 
    Rhos =   Vp2rho(Vps)
    Thicks = np.concatenate([np.array([Thickness_crust-thickness_cl]),np.array([0])]) 
print(Vps)
print(Vss)
print(Thicks)
Vp = Vps.astype('float')
...
SynSavePath1 = f"{SynSavePath}/sediment_depth0"
